...
const Chart: React.FC<ChartProps> = ({ 
    ...
    try {
      switch (type) {
        ...
        case 'bar':
          ...
          drawBarChart(ctx, barData, chartWidth, chartHeight, margin, textColor, gridColor);
          break;
        case 'area':
        ...
        case 'pie':
          ...
          drawPieChart(ctx, pieData, Math.min(chartWidth, chartHeight), textColor);
          break;
        case 'donut':
          const donutData = ensureChartData(data);
          drawDonutChart(ctx, donutData, Math.min(chartWidth, chartHeight), textColor);
          break;
      }
    } catch (error) {
  ...
  const ensureChartData = (inputData: ChartData[] | LineChartData[] | Dataset[]): ChartData[] => {
    ...
    if ('id' in inputData[0]) {
      ...
      if (datasets[0] && datasets[0].data) {
        return datasets[0].data.map((item, index) => ({
          ...
          value: item.y,
          color: datasets[0].color || defaultConfig.colors[index % defaultConfig.colors.length]
        }));
      }
    }
  ...
  const drawLineChart = (
    ...
    if (datasets.length === 0 || !datasets[0].data || datasets[0].data.length === 0) return;
  ...
  const drawRoundedDonutSlice = (
    ...
    ctx.moveTo(centerX + Math.cos(startAngle) * outerRadius, centerY + Math.sin(startAngle) * outerRadius);
